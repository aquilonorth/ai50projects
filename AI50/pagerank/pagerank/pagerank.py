...
def iterate_pagerank(corpus, damping_factor):
    ...
    adjusted_corpus = {
        page: links if links else set(corpus.keys())
    ...
    while not converged:
        ...
        for page in corpus:
            ...
            for other_page in corpus:
                if page in adjusted_corpus[other_page]:
                    total += pagerank[other_page] / len(adjusted_corpus[other_page])
            new_ranks[page] = (1 - damping_factor) / num_pages + damping_factor * total

        converged = all(abs(new_ranks[page] - pagerank[page]) < threshold for page in corpus)
        pagerank = new_ranks
